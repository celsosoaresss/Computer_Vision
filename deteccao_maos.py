...
def imprime_botoes(img, posicao, letra, tamanho = 50, cor_retangulo = BRANCO):
    ...
    cv2.rectangle(img, posicao, (posicao[0] + tamanho, posicao[1]+tamanho), BRANCO, cv2.FILLED)
...
while True:
    ...
    if len(todas_maos) ==1:
        ...
        if todas_maos[0]['lado'] == 'Right':
            ...
            if info_dedos_maos1 == [False, False, False, False, True] and len(texto)>1:
                texto = texto[:-1]
                sleep(0.15)
        ...
        if todas_maos[0]['lado'] == 'Left':
            if info_dedos_maos1 == [False, True, False, False, False] and bloco_notas == False:
                bloco_notas = True
                os.startfile(r'C:\Windows\System32\notepad.exe')

            if info_dedos_maos1 == [False, True, True, False, False] and chrome == False:
                chrome = True
                os.startfile(r'C:\Program Files\Google\Chrome\Application\chrome.exe')

            if info_dedos_maos1 == [False, True, True, True, False] and calculadora == False:
                calculadora = True
                os.startfile(r'C:\Windows\System32\calc.exe')

            if info_dedos_maos1 == [False, False, False, False, False] and bloco_notas == True:
                bloco_notas = False
                os.system('TASKKILL /IM notepad.exe')

            if info_dedos_maos1 == [False, True, False, False, True]:
                break

    if len(todas_maos) ==2:
        info_dedos_maos1 = dedos_levantados(todas_maos[0])
        info_dedos_maos2 = dedos_levantados(todas_maos[1])

        indicador_x, indicador_y, indicador_z = todas_maos[0]['coordenadas'][8]

        if sum(info_dedos_maos2) ==1:
            cor_pincel = AZUL
        elif sum(info_dedos_maos2) ==2:
            cor_pincel = VERDE
        elif sum(info_dedos_maos2) ==3:
            cor_pincel = VERMELHO
        elif sum(info_dedos_maos2) ==4:
            cor_pincel = BRANCO  
        else:
            img_quadro = np.ones((resolucao_y, resolucao_x, 3), np.uint8)*255
        ...
        if info_dedos_maos1 == [False, True, False, False, False]:
